          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
                ...
                {user && (
                  isAnySyncing ? (
                    ...
                    >
                      ...
                      <span>
                        Syncing {syncingCount > 0 ? `${syncingCount} ${syncingCount === 1 ? "entry" : "entries"}` : "with Cloud"}...
                      </span>
                    </span>
                  ) : (
              ...
              <p className="text-xs text-slate-400">
                {entries.length} scanned {entries.length === 1 ? "landmark" : "landmarks"}{" "}
                {user ? `persisted in your Firestore account` : `stored locally in your browser`}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {entries.length === 0 ? (
            <div className="py-16 text-center text-slate-500">
              ...
              <p className="text-sm font-medium text-slate-400">No scanned landmarks yet</p>
              <p className="text-xs text-slate-500 mt-1">
                Take or upload a photo to start your AR travel diary.
              </p>
            </div>
          ) : (
            entries.map((entry) => {
